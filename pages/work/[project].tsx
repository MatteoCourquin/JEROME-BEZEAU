  ...
  return (
    <>
      <section className="overflow-hidden pt-header">
        <div className="grid grid-cols-1 gap-y-default gap-x-5 px-x-default py-y-default lg:grid-cols-12">
          ...
          {project.tags && (
            <div className="smoother-x-black absolute -left-x-default col-span-6 w-screen sm:left-0 sm:block">
              <div className="no-scrollbar flex gap-2 overflow-scroll px-x-default sm:px-0">
          ...
          <RichText
            className="col-span-6 pt-y-half-default"
            value={isFrench ? project.descriptionFr : project.descriptionEn}
          />
          {project.credits && (
            <div className="col-span-4 -col-end-1 row-start-4 flex flex-col lg:row-span-2 lg:row-start-2">
              <p className="pb-10">CREDITS :</p>
              <ul className="flex flex-col gap-3">
                {project.credits.map((credit, index) => (
                  <li
                    key={index}
                    className="flex items-center gap-x-3 transition-[padding-left] hover:pl-3"
                  >
                    <span className="uppercase text-white-40">
                      {isFrench ? credit.role.labelFr : credit.role.labelEn}
                    </span>
                    <div className="h-px min-w-4 grow bg-white-12"></div>
                    {credit.author.websiteUrl ? (
                      <a
                        className="link link_white-80 cursor-button overflow-hidden text-ellipsis whitespace-nowrap"
                      ...
                      <p className="overflow-hidden text-ellipsis whitespace-nowrap text-white-40">
                        {isSmallMobile ? getFirstName(credit.author.name) : credit.author.name}
                      </p>
                    )}
                  </li>
